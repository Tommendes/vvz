    ...
    <div class="bg-surface-50 dark:bg-surface-950 flex items-center justify-center min-h-screen min-w-[100vw] overflow-hidden">
        <div class="flex flex-col items-center justify-center">
            <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                <div class="w-full bg-surface-0 dark:bg-surface-900 p-5" style="border-radius: 53px">
                    <div class="text-center mb-8">
                        <img src="/images/bot.png" alt="Vivazul logo" class="mb-8 w-20 shrink-0 mx-auto" />
                        <div class="text-surface-900 dark:text-surface-0 text-3xl font-medium mb-4">Bem vindo ao Vivazul!</div>
                        <span v-if="isToken" class="text-muted-color font-medium">Preencha abaixo para recuperar sua senha</span>
                    ...
                    <Fluid v-if="isToken && tokenTimeLeft > 0">
                        <label for="token" class="block text-surface-900 dark:text-surface-0 text-xl font-medium">Seu token</label>
                    ...
                    <div class="flex items-center justify-between mt-2 mb-8 gap-8">
                        <span class="font-medium no-underline ml-2 text-right cursor-pointer text-primary" @click="router.push({ name: 'request-password-reset' })">Esqueceu sua senha?</span>
                        <span class="font-medium no-underline ml-2 text-right cursor-pointer text-primary" @click="router.push({ name: 'signin' })">Acessar</span>
                    </div>

                    <div v-if="isToken">
                        <Button v-if="tokenTimeLeft > 0" label="Confirmar e acessar o sistema" icon="fa-solid fa-arrow-right-to-bracket" :disabled="token && token.length != 8" @click="unlock" class="w-full p-3 text-xl mt-3 mb-3 gap-5"></Button>
                        <Button v-else label="Solicite outro token" icon="fa-solid fa-arrow-right-to-bracket" @click="getNewToken" class="w-full p-3 text-xl mt-3 mb-3 gap-5"></Button>
                    </div>
                    <Button v-else label="Para home" icon="fa-solid fa-arrow-right-to-bracket" @click="router.push('/')" class="w-full p-3 text-xl mt-3 mb-3 gap-5"></Button>
                </div>
            </div>
        </div>
    </div>
