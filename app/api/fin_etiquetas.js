...
module.exports = app => {
    ...
    const save = async (req, res) => {
        ...
        try {
            ...
            const unique = await app.db(tabelaDomain).where({ id_asc: body.id_asc, descricao: body.descricao }).first()
            if (unique && unique.id != body.id) throw 'Descrição já cadastrada'
        } catch (error) {
    ...
    const getListFamily = async (req, res) => {
        let user = req.user
        const uParams = await app.db({ u: 'users' }).join({ sc: 'schemas_control' }, 'sc.id', 'u.schema_id').where({ 'u.id': user.id }).first();
        try {
            // Alçada do usuário
        ...
        const way = (req.query.way && ['1', '2'].includes(req.query.way)) ? `id = ${req.query.way}` : 'id_asc is null'
        const tabelaDomain = `${dbPrefix}_${uParams.schema_name}.${tabela}`
        try {
            const list = await app.db.raw(`WITH RECURSIVE cte_centros AS (
                SELECT id, id_asc, descricao AS tag_name, descricao, CAST(descricao AS CHAR(1000)) AS caminho
                FROM ${tabelaDomain}
                WHERE ${way}
                UNION ALL        
                SELECT fe.id, fe.id_asc, fe.descricao AS tag_name, fe.descricao, CONCAT(cte.caminho, ',', fe.descricao) 
                FROM ${tabelaDomain} fe 
                INNER JOIN cte_centros cte ON fe.id_asc = cte.id
              )
              SELECT id, caminho, tag_name FROM cte_centros ORDER BY id`)
            const count = list[0].length
            return res.json({ data: list[0], count })
        } catch (error) {
