...
module.exports = app => {
    ...
    const tabela = 'mensagens_sistema'
    ...
    const save = async (req, res) => {
        ...
        try {
            // Alçada para edição
            if (body.id)
                isMatchOrError(uParams && uParams.orgao >= 1, `${noAccessMsg} "Edição de ${tabela}"`)
            // Alçada para inclusão
            else isMatchOrError(uParams && uParams.orgao >= 1, `${noAccessMsg} "Inclusão de ${tabela}"`)
        } catch (error) {
        ...
        try {
            ...
            existsOrError(body.expira, 'Expira não informado')
        } catch (error) {
        ...
        if (body.id) {
            ...
            rowsUpdated.then((ret) => {
                ...
                else res.status(200).send('Mensagens do Sistema não foi encontrada')
            })
                .catch(error => {
