...
module.exports = app => {
    ...
    const save = async (req, res) => {
        ...
        try {
            existsOrError(body.file_permission, 'Permissão não informada')
            existsOrError(body.file_name, 'Nome não informado')
            existsOrError(body.file_caption, 'Legenda não informada')
            existsOrError(body.file_size, 'Tamanho não informado')
            existsOrError(body.file_ext, 'Extensão não informada')        
            existsOrError(body.file_wd, 'Largura não informada')        
        } catch (error) {
        ...
        if (body.id) {
            ...
            rowsUpdated.then((ret) => {
                ...
                else res.status(200).send('Upload não foi encontrado')
            })
                .catch(error => {
    ...
    const get = async (req, res) => {
        ...
        if (key)
            if (keyCnpj) count.where(function () {
                this.where(app.db.raw(`tbl1.file_size like '%${keyCnpj}%'`))
                    .orWhere(app.db.raw(`tbl1.file_name regexp('${key.toString().replace(' ', '.+')}')`))
            })
            else count.where(app.db.raw(`tbl1.file_name regexp('${key.toString().replace(' ', '.+')}')`))
        ...
        if (key)
            ...
            else ret.where(app.db.raw(`tbl1.file_name regexp('${key.toString().replace(' ', '.+')}')`))

        ret.where({ status: STATUS_ACTIVE })
            .groupBy('tbl1.id')
            .orderBy('file_name', 'file_size')
