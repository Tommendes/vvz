...
module.exports = app => {
    ...
    const save = async (req, res) => {
        ...
        try {
            ...
            app.api.logger.logError({ log: { line: `Error in access file: ${__filename} (${__function}). User: ${uParams.name}. User: ${uParams.name}. Error: ${error}`, sConsole: true } })
        ...
        if (body.id) {
            ...
            const evento = await createEventUpd({
                ...
                "request": req,
                "evento": {
                    "evento": `Alteração de  ${tabela.charAt(0).toUpperCase() + tabela.slice(1).replaceAll('_', ' ')}`,
                    "tabela_bd": tabela,
                }
            })
