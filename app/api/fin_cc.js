...
module.exports = app => {
    ...
    const tabela = 'fin_cc'
    const tabelaAlias = 'Centro de Custo'
    ...
    const save = async (req, res) => {
        ...
        try {
            ...
            existsOrError(body.codigo, 'Código não encontrado')
            existsOrError(body.tipo, 'Tipo não encontrado')
            if (body.tipo.length = !1) throw "Tipo inválido"
            existsOrError(body.descricao, 'Descrição não encontrada')
    ...
    const remove = async (req, res) => {
        ...
        try {
            // Alçada do usuário
            isMatchOrError(uParams && uParams.financeiro >= 4, `${noAccessMsg} "Exclusão de ${tabelaAlias}"`)
        } catch (error) {
            app.api.logger.logError({ log: { line: `Error in access file: ${__filename} (${__function}). Error: ${error}`, sConsole: true } })
            return res.status(401).send(error)
        }
