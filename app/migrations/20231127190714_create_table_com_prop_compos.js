...
exports.up = function (knex) {
    return knex.schema.createTable(migrationClientSchema + '.com_prop_compos', table => {
        table.engine('InnoDB')
        ...
        table.integer('id_com_propostas').unsigned().references('id').inTable(migrationClientSchema + '.com_propostas').onUpdate('Cascade').onDelete('NO ACTION').comment('Chave estrangeira com a tabela com_propostas')
        table.boolean('comp_ativa').defaultTo(1).notNull().comment('Composição ativa (Não:0; Sim:1)')
...
exports.down = function (knex) {
    return knex.schema.dropTable('vivazul_bceaa5.com_prop_compos')
};
