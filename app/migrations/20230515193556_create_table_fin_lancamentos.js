exports.up = function(knex, Promise) {
    return knex.schema.createTable('vivazul_cso_root.fin_lancamentos', table => {
        ...
        table.integer('id_empresa',10).unsigned().notNull().defaultTo(0).comment('Codigo relacional com a tabela sis_demp')
        table.integer('id_cadastros',10).unsigned().notNull().defaultTo(0).comment('Codigo relacional com a tabela de cadastros')
        table.integer('id_centro_custo',10).unsigned().notNull().defaultTo(0).comment('Codigo relacional com a tabela fin_centro_custo')
        table.string('tipoDocumento',255).comment('Documento gerado: CF, NF-e, NFS-e')
        table.string('doc_fiscal',255).defaultTo(0).comment('Nota fiscal do lancamento')
        ...
        table.double('valor_bruto',11,2).defaultTo(0.00).comment('Valor bruto da conta')
        table.double('valor_bruto_nf',11,2).defaultTo(0.00).comment('Valor bruto da nota fiscal')
        table.double('valor_retencao',11,2).defaultTo(0.00).comment('Valor da retenção. Este campo será impresso na duplicata.')
        table.double('valor_liquido',11,2).defaultTo(0.00).comment('Valor liquido da conta')
        ...
        table.double('valor_vencimentos',11,2).defaultTo(0.00).comment('Valor bruto da conta')
        table.double('valor_nota_fiscal',11,2).defaultTo(0.00).comment('Valor total da nota fiscal para impressão na duplicata')
        table.string('duplicata',255).comment('Duplicata da conta gerada no faturamento')
        table.integer('duplicata_impr',1).defaultTo(0).comment('Quantidade de vezes que a duplicata foi impresso')
