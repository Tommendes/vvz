...
exports.up = function(knex, Promise) {
    return knex.schema.createTable(migrationClientSchema + '.fis_notas', table => {
        ...
        table.string('modelo').notNull().comment('Modelo da nota')
        ...
        table.string('data_emissao').notNull().comment('Data de emissão')
        table.specificType('mov_e_s', char(1)).notNull().comment('Movimento de entrada/saída')
        table.specificType('modelo_nf', char(2)).notNull().comment('Modelo da nota')
        ...
        table.integer('id_fornecedor').notNull().unsigned().references('id').inTable(migrationClientSchema + '.cadastros').onUpdate('Cascade').onDelete('NO ACTION').comment('Fornecedor relacionado')
        table.string('descricao', 1000).comment('Descrição da nota')
