...
exports.up = function(knex, Promise) {
    return knex.schema.createTable(migrationClientSchema + '.fin_lancamentos', table => {
        ...
        table.integer('id_cadastros').notNull().unsigned().references('id').inTable(migrationClientSchema + '.cadastros').onUpdate('Cascade').onDelete('NO ACTION').comment('Cadastro relacionado')
        table.integer('id_tipo_documento').notNull().unsigned().references('id').inTable(migrationClientSchema + '.local_params').onUpdate('Cascade').onDelete('NO ACTION').comment('Tipo de documento')
        table.integer('id_pipeline').unsigned().references('id').inTable(migrationClientSchema + '.pipeline').onUpdate('Cascade').onDelete('NO ACTION').comment('Pipeline relacionado')
        table.integer('id_comissoes').unsigned().references('id').inTable(migrationClientSchema + '.comissoes').onUpdate('Cascade').onDelete('NO ACTION').comment('Comissão relacionada')
        table.integer('id_notas').unsigned().references('id').inTable(migrationClientSchema + '.fis_notas').onUpdate('Cascade').onDelete('NO ACTION').comment('Nota relacionada')
        ...
        table.integer('id_fin_conciliacao').unsigned().references('id').inTable(migrationClientSchema + '.fin_conciliacao').onUpdate('Cascade').onDelete('NO ACTION').comment('Conciliação relacionada')
