...
-- Totais
SELECT 'receita',COUNT(cod) FROM mygsoft.fin_lancamentos WHERE situacao >= 1 AND valor_bruto > 0 -- 33778
UNION ALL
SELECT 'despesa',COUNT(cod) FROM mygsoft.fin_lancamentos WHERE situacao >= 1 AND valor_bruto < 0 -- 30254
UNION ALL
SELECT 'total',COUNT(cod) FROM mygsoft.fin_lancamentos; -- 64032

-- Importar lançamentos 4:37 minutos
SET FOREIGN_KEY_CHECKS = 0;
TRUNCATE TABLE vivazul_bceaa5.fin_lancamentos;
UPDATE mygsoft.fin_lancamentos fl SET fl.`valor_bruto` = 0 WHERE fl.`valor_bruto` = '';
UPDATE mygsoft.fin_lancamentos fl SET fl.`valor_bruto_nf` = 0 WHERE fl.`valor_bruto_nf` = '';
UPDATE mygsoft.fin_lancamentos fl SET fl.`valor_liquido` = 0 WHERE fl.`valor_liquido` = '';
UPDATE mygsoft.fin_lancamentos fl SET fl.`valor_nota_fiscal` = 0 WHERE fl.`valor_nota_fiscal` = '';
UPDATE mygsoft.fin_lancamentos fl SET fl.`valor_retencao` = 0 WHERE fl.`valor_retencao` = '';
UPDATE mygsoft.fin_lancamentos fl SET fl.`valor_vencimentos` = 0 WHERE fl.`valor_vencimentos` = '';
...
INSERT INTO vivazul_bceaa5.fin_parcelas (id,evento,created_at,updated_at,STATUS,situacao,id_fin_lancamentos,data_vencimento,data_pagto,
	valor_vencimento,
	duplicata,parcela,recorrencia,descricao,documento,id_fin_contas,motivo_cancelamento)
(
	...
	flv.id id_fin_lancamentos,fl.data_vencimento,fl.data_pagto, 
	(flv.valor_bruto - (SELECT COALESCE(SUM(valor_retencao),0) FROM vivazul_bceaa5.fin_retencoes fr WHERE fr.id_fin_lancamentos = flv.id)) fp, 
	fl.duplicata, fl.vencimento, 'U', fl.obs_da_conta, fl.doc_pagto,
