...
ALTER TABLE vivazul_bceaa5.cadastros CHANGE pgi_id pgi_id INT(10) UNSIGNED NOT NULL, DROP PRIMARY KEY, ADD PRIMARY KEY (id, old_id);
ALTER TABLE vivazul_bceaa5.cadastros DROP COLUMN IF EXISTS pgi_id;
ALTER TABLE vivazul_bceaa5.cadastros ADD COLUMN pgi_id INT(11) NULL;
ALTER TABLE vivazul_bceaa5.cadastros CHANGE pgi_id pgi_id INT(10) UNSIGNED NOT NULL, DROP PRIMARY KEY, ADD PRIMARY KEY (id, old_id, pgi_id);
...
WHERE cv.id IS NULL
GROUP BY c.cod;
-- Inserir cadastros inexistentes: aprox 7minutos
...
IF(LENGTH(TRIM(c.cpf_cnpj)) < 11 OR CAST(c.cpf_cnpj AS UNSIGNED) = 0, CONCAT('9',LPAD(REPLACE(RAND(UNIX_TIMESTAMP(CURRENT_TIMESTAMP)),'.',''), 13, '0')), c.cpf_cnpj)cpf_cnpj,c.rg_ie,c.estab,
c.sexo,c.nascimento,4,c.cim,c.doc_esp,c.mala,c.telefone1,c.email,c.obs,52,SUBSTRING(REPLACE(REPLACE(c.cep1,'-',''),'.',''),0,8)cep,c.logradouro1,c.nr1,c.complnr1,c.bairro1,c.cidade1,c.estado1,NULL,NULL,NULL,0,c.cod
...
-- Editar cadastros existentes para pgi_id: 17/09 = 12577 registros de um total de 13396 registros em vivazul_bceaa5.cadastros
...
order by cpf_cnpj;
*/
-- Contar cadastros em fin_lancamentos = 8612 em 17/09
-- select count(cod_cadas) from mygsoft.fin_lancamentos group by cod_cadas;
SET FOREIGN_KEY_CHECKS = 1;
...
-- Totais
SELECT 'receita',COUNT(cod) FROM mygsoft.fin_lancamentos WHERE situacao >= 1 AND valor_bruto > 0 -- 33041
UNION ALL
SELECT 'despesa',COUNT(cod) FROM mygsoft.fin_lancamentos WHERE situacao >= 1 AND valor_bruto < 0 -- 30110
UNION ALL
SELECT 'total',COUNT(cod) FROM mygsoft.fin_lancamentos; -- 63151
...
-- Totais
SELECT 'receita',COUNT(id) FROM vivazul_bceaa5.fin_lancamentos WHERE centro = 1 -- 33041
UNION ALL
SELECT 'despesa',COUNT(id) FROM vivazul_bceaa5.fin_lancamentos WHERE centro = 2 -- 30110
UNION ALL
SELECT 'total',COUNT(id) FROM vivazul_bceaa5.fin_lancamentos; -- 63515
