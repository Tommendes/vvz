...
const loadParcelas = async () => {
    ...
    itemDataParcelas.value = await axios.get(url).then((res) => {
        mode.value = 'grid';
        ...
        <Fieldset :toggleable="true" :collapsed="false" class="mb-1">
            ...
            <div class="flex justify-content-between flex-wrap mb-1">
                <div class="flex align-items-start">
                    <Button type="button" v-if="props.itemDataRoot.id" severity="contrast" rounded size="large" icon="fa-solid fa-plus fa-shake" label="Adicionar Vencimento" @click="setNewItem()" />
                </div>
                <div class="flex justify-content-end">
                    <div v-if="missingValue || (itemDataParcelas.data && itemDataParcelas.data.length)">
                        <h4 v-if="itemDataParcelas.data && itemDataParcelas.data.length" class="mt-1">Soma total das parcelas: {{ formatCurrency(itemDataParcelas.total) }}</h4>
                        <h4 v-if="itemDataParcelas.emAtraso && itemDataParcelas.data && itemDataParcelas.data.length" class="mt-1 text-red-500">Em atraso: {{ formatCurrency(itemDataParcelas.emAtraso) }}</h4>
                        <h4 v-if="itemDataParcelas.estaSemana && itemDataParcelas.estaSemana.length && itemDataParcelas.data && itemDataParcelas.data.length" class="mt-1 text-orange-500" v-for="item in itemDataParcelas.estaSemana" :key="item">
                            {{ item.labelData }} esta semana: {{ item.data }} {{ formatCurrency(item.valor) }}
                        </h4>
                        <h4 v-if="itemDataParcelas.aVencer && itemDataParcelas.data.length" class="mt-1 text-green-500">Total das parcelas a vencer: {{ formatCurrency(itemDataParcelas.aVencer) }}</h4>
                        <h4 v-if="itemDataParcelas.pago && itemDataParcelas.data.length" class="mt-1 text-primary-500">Total das parcelas pagas: {{ formatCurrency(itemDataParcelas.pago) }}</h4>

                        <h4 v-if="missingValue && itemDataParcelas.data && itemDataParcelas.data.length" class="mt-0">
                            <span class="border-bottom-1">Valor liquido deste registro: {{ formatCurrency(props.totalLiquido) }}</span>
                        </h4>
                        <h4 v-if="missingValue" class="mt-0 text-orange-700">{{ missingValue < 0 ? 'Falta registrar o valor de' : 'Valor registrado a maior' }}: {{ formatCurrency(missingValue < 0 ? missingValue * -1 : missingValue) }}</h4>
                    </div>
                </div>
            </div>
            <ParcelaItem v-if="mode == 'new'" :mode="mode" :itemData="itemData" @cancel="cancel" @reloadItems="loadParcelas" :uProf="props.uProf" :itemDataRoot="props.itemDataRoot" />
            ...
            <Fieldset
                ...
                v-for="item in itemDataParcelas.data"
                :key="item.id"
            >
                <template #legend>
                    <div class="flex align-items-center text-primary">
                        <span class="fa-solid fa-bolt mr-2"></span>
                        <span :class="`font-bold text-lg ${setAdicionalVencimentoClass(item).class}`">{{ setAdicionalVencimentoClass(item).label }}. {{ item.situacaoLabel }}</span>
                    </div>
                </template>
                <ParcelaItem :itemData="item" @cancel="cancel" @reloadItems="loadParcelas" :uProf="props.uProf" :itemDataRoot="props.itemDataRoot" />
