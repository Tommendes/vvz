...
const getEmpresas = async () => {
    ...
    await axios.get(url).then((res) => {
        ...
        if (uProf.value.id_empresa && uProf.value.multiCliente < 1) empresa.value = uProf.value.id_empresa;
            ...
            dropdownEmpresas.value = [{ value: '0', label: 'Selecione aqui uma empresa para exibir os registros (Ou esta opção para todas...)' }, ...dropdownEmpresas.value];
            empresa.value = '0';
        }
        ...
        columns: [
            ...
            },
            {
                ...
                value: (row) => {
                    ...
                    switch (answer) {
                        case STATUS_REGISTRO_ABERTO:
                            answer = 'Aberto';
                            break;
                        case STATUS_REGISTRO_PAGO:
                            answer = 'Pago';
                            break;
                        case STATUS_REGISTRO_CONCILIADO:
                            answer = 'Conciliado';
                            break;
                        case STATUS_REGISTRO_CANCELADO:
                            answer = 'Cancelado';
                            break;
                        default:
                            answer = 'N/D';
            ...
            { label: 'Vencimento em', value: (row) => (row.data_vencimento ? row.data_vencimento : 'Não informado') },
            { label: 'Pagamento em', value: (row) => (row.data_pagto ? row.data_pagto : 'Não informado') },
                    ...
                    <template #header>
                        ...
                        <div class="flex justify-content-end gap-3 mb-3 p-tag-esp">
                            ...
                            <!-- <SplitButton icon="fa-solid fa-cloud-arrow-down" label="Exportar dados" :model="itemsExport"
                                @click="exportPdf()" /> -->
                            <Button type="button" icon="fa-solid fa-file-pdf" label="Exportar dados" outlined
                                @click="exportPdf()" />
