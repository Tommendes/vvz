...
const itemsButtons = ref([
    {
        ...
        command: () => {
            router.push({ path: `/${uProf.value.schema_description}/retencao/${itemData.value.id}` });
        }
    },
    {
        label: 'Excluir',
...
<template>
    ...
    <div class="card">
        ...
        <DataTable style="font-size: 1rem" :value="gridData" :paginator="true" :rows="10" dataKey="id" :rowHover="true"
            ...
            responsiveLayout="scroll" :globalFilterFields="['id_fin_lanc', 'valor', 'valor_da_retencao']">
            <template #header>
