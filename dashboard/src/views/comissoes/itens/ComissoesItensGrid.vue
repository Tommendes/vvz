...
// Agendar liquidação em grupo
const setGroupBilled = () => {
    confirm.require({
        group: 'comisGroupBilledConfirm',
        header: 'Confirmar faturamento para todos',
        message: [
            `Confirma o faturamento deste${pendingCommissionsQuantity() > 1 ? 's ' + pendingCommissionsQuantity() : ''} registro${pendingCommissionsQuantity() > 1 ? 's' : ''}?`,
            'Essa operação ainda poderá ser revertida individualmente enquanto não houver o encerramento.'
        ],
        icon: 'fa-solid fa-question fa-beat',
        acceptIcon: 'fa-solid fa-check',
        rejectIcon: 'fa-solid fa-xmark',
...
// hasPendingBill 
const hasPendingBill = () => {
    return true; //gridData.value.some((item) => item.last_status_comiss < 40);
};
// Executar liquidação em grupo
...
const hasPendingCommissions = () => {
    return true; //gridData.value.some((item) => item.last_status_comiss < 20);
};
...
<template>
    ...
    <ConfirmDialog group="comisGroupBilledConfirm">
        <template #container="{ message, acceptCallback, rejectCallback }">
            <div class="flex flex-column align-items-center p-5 surface-overlay border-round">
                <div
                    class="border-circle bg-primary inline-flex justify-content-center align-items-center h-6rem w-6rem -mt-8">
    ...
    <div class="flex justify-content-end gap-3 mb-5">
        ...
        <Button v-if="gridData && gridData.length > 0" type="button" icon="fa-solid fa-file-invoice-dollar"
            label='"Liquidado" para todos' :outlined="hasPendingCommissions()" :disabled="!hasPendingCommissions()"
            @click="scheduleGroupSettlement()"
            v-tooltip.top="'Clique para liberar o pagamento de todas as comissões pendentes'" />
        <Button v-if="gridData && gridData.length > 0" severity="success" outlined type="button"
            icon="fa-solid fa-check" label='"Faturado" para todos' :disabled="!hasPendingBill()"
            @click="setGroupBilled()"
            v-tooltip.top="'Clique para confirmar o faturamento para todas as comissões devidas'" />
        <!-- <Button
