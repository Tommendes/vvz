        ...
        if (route.params.id || itemData.value.id) {
            ...
            await axios.get(url).then(async (res) => {
                ...
                if (body && body.id) {
                    ...
                    router.push({ path: `/${userData.cliente}/${userData.dominio}/prop-composicoes` });
                }
            });
        ...
        axios[method](url, itemData.value)
            .then((res) => {
                ...
                if (body && body.id) {
                    ...
                    itemData.value = body;
                    if (mode.value == 'new') router.push({ path: `/${userData.cliente}/${userData.dominio}/prop-composicao/${itemData.value.id}` });
                    dataRegistro.value = moment(itemData.value.updated_at || itemData.value.created_at).format('DD/MM/YYYY HH:mm:ss');
                    mode.value = 'view';
                } else {
...
<template>
    <Breadcrumb v-if="mode != 'new'" :items="[{ label: 'Todas as Composições', to: `/${userData.cliente}/${userData.dominio}/prop-composicoes` }, { label: itemData.id_com_propostas + (userData.admin >= 1 ? `: (${itemData.id})` : '') }]" />
